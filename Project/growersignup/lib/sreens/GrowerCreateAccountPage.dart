...
class GrowerCreateAccountPage extends StatefulWidget {
  const GrowerCreateAccountPage({super.key});

  @override
  State<GrowerCreateAccountPage> createState() => _GrowerCreateAccountPageState();
}
...
class _GrowerCreateAccountPageState extends State<GrowerCreateAccountPage> {
  ...
  static const Color dropdownBackgroundColor = Color(0xFFE8F5E9); // Slightly adjusted dropdown green
         ...
         return Theme(
           data: ThemeData.light().copyWith(
             colorScheme: const ColorScheme.light(
               primary: buttonColor, onPrimary: Colors.white, onSurface: Colors.black,
             ),
  ...
  void _createAccount() {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      final accountData = {
        ...
        'address2': _address2Controller.text.isNotEmpty ? _address2Controller.text : null, // Send null if empty? Check API reqs
        'city': _cityController.text,
        'postalCode': _postalCodeController.text,
        'gender': _selectedGender,
        'dateOfBirth': _selectedDate != null ? DateFormat('yyyy-MM-dd').format(_selectedDate!) : null, // Format for API
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar( // Keep AppBar for navigation context
        backgroundColor: pageBackgroundColor,
        elevation: 0,
        leading: IconButton( // Allow user to go back if needed
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildTextField(
                      controller: _firstNameController, hintText: 'Enter First name',
                      validator: (v) => v == null || v.isEmpty ? 'Required' : null,
                      textCapitalization: TextCapitalization.words,
                    ),
                    const SizedBox(height: 10),
                    _buildTextField(
                      controller: _lastNameController, hintText: 'Enter Last name',
                      validator: (v) => v == null || v.isEmpty ? 'Required' : null,
                      textCapitalization: TextCapitalization.words,
                    ),
                    ...
                    _buildTextField(
                      controller: _nicController, hintText: 'Enter NIC number',
                      validator: (v) => v == null || v.isEmpty ? 'Required' : null,
                    ...
                    _buildLabel('Address:'),
                    _buildTextField(
                      controller: _address1Controller, hintText: 'Address line 1',
                      validator: (v) => v == null || v.isEmpty ? 'Required' : null,
                      textCapitalization: TextCapitalization.words,
                    ),
                    const SizedBox(height: 10),
                    _buildTextField(
                      controller: _address2Controller, hintText: 'Address line 2 (Optional)',
                       textCapitalization: TextCapitalization.words,
                    ...
                    _buildTextField(
                      controller: _phoneController, hintText: '+94 - 000000000',
                      keyboardType: TextInputType.phone,
                      inputFormatters: [FilteringTextInputFormatter.allow(RegExp(r'[0-9\s\-\+]'))],
                      validator: (v) {
                        if (v == null || v.isEmpty) return 'Required';
                        if (v.length < 10) return 'Invalid phone number'; // Example
                        return null;
                      },
                    ),
                    const SizedBox(height: 20),
  ...
  Widget _buildTextField({
    ...
    return TextFormField(
      ...
      obscureText: obscureText, inputFormatters: inputFormatters,
  ...
  Widget _buildDateField(BuildContext context) {
    return TextFormField(
      ...
      decoration: InputDecoration(
         hintText: 'Year/Mon/Date', hintStyle: TextStyle(color: Colors.grey[500], fontSize: 14),
         filled: true, fillColor: Colors.white,
         suffixIcon: Icon(Icons.calendar_today_outlined, color: inputBorderColor.withOpacity(0.7)),
          border: OutlineInputBorder(
             borderRadius: BorderRadius.circular(12.0),
             borderSide: BorderSide(color: inputBorderColor.withOpacity(0.5)),
           ),
      ...
      onTap: () { FocusScope.of(context).requestFocus(FocusNode()); _selectDate(context); },
  ...
  Widget _buildDropdown({
    ...
    return DropdownButtonFormField<String>(
      value: value, items: items.map((item) => DropdownMenuItem<String>(value: item, child: Text(item, style: const TextStyle(color: dropdownTextColor, fontSize: 14)))).toList(),
      onChanged: onChanged,
      decoration: InputDecoration(
         hintText: hint, hintStyle: TextStyle(color: Colors.grey[600], fontSize: 14),
         filled: true, fillColor: dropdownBackgroundColor,
         border: OutlineInputBorder(borderRadius: BorderRadius.circular(12.0), borderSide: BorderSide.none,), // Changed to match text field border
          enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12.0), borderSide: BorderSide.none,),
          focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12.0), borderSide: const BorderSide(color: focusedInputBorderColor, width: 1.5),),
          errorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12.0), borderSide: const BorderSide(color: Colors.redAccent, width: 1.0),),
          focusedErrorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12.0), borderSide: const BorderSide(color: Colors.redAccent, width: 1.5),),
         contentPadding: const EdgeInsets.symmetric(vertical: 14.0, horizontal: 15.0), // Match text field padding
         isDense: true,
      ),
      dropdownColor: dropdownBackgroundColor.withOpacity(0.95),
      icon: Icon(Icons.keyboard_arrow_down, color: dropdownTextColor.withOpacity(0.8)),
