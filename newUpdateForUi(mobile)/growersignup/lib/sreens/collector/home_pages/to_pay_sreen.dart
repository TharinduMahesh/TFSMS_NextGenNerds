...
import 'package:growersignup/sreens/collector/home_pages/to_pay_screen_details.dart';
...
class PendingPaymentsScreen extends StatefulWidget {
  final String email;
  const PendingPaymentsScreen({super.key, required this.email});

  @override
  State<PendingPaymentsScreen> createState() => _PendingPaymentsScreenState();
}

class _PendingPaymentsScreenState extends State<PendingPaymentsScreen> with TickerProviderStateMixin {
  late Future<List<PendingPayment>> _paymentsFuture;
  ...
  void initState() {
    ...
    _paymentsFuture = _apiService.getPendingCashPayments(_collectorEmail);
  ...
  Widget _buildPaymentsList(List<PendingPayment> payments) {
    return Column(
      children: [
        // Header Section
        Container(
  ...
  Widget _buildPaymentCard(PendingPayment payment, int index) {
    return Container(
