...
class GPendingOrdersPage extends StatefulWidget {
  final String email;
  const GPendingOrdersPage({super.key, required this.email});

  @override
  State<GPendingOrdersPage> createState() => _GPendingOrdersPageState();
}

class _GPendingOrdersPageState extends State<GPendingOrdersPage> with TickerProviderStateMixin {
  late Future<List<GPendingOrder>> _ordersFuture;
  ...
  static const Color warningOrange = Color.fromARGB(255, 26, 125, 82);

  @override
  void initState() {
    super.initState();
    _ordersFuture = GOrderApiService.getPendingOrders();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<GPendingOrder>>(
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.pending_actions, color: warningOrange, size: 50),
            ),
            const SizedBox(height: 20),
            Text(
              'No Pending Orders',
            ...
            const SizedBox(height: 10),
            Text(
              'You have no pending order requests at the moment.',
              textAlign: TextAlign.center,
              style: TextStyle(color: textLight, fontSize: 14),
            ),
            const SizedBox(height: 20),
  ...
  Widget _buildOrdersList(List<GPendingOrder> orders) {
    return Column(
      children: [
        // Header Section
        Container(
          ...
          child: Row(
            children: [
              ...
              Text(
                '${orders.length} Pending Request${orders.length > 1 ? 's' : ''}',
                style: TextStyle(
                  fontSize: 18,
                  fontWeight: FontWeight.bold,
                  color: textDark,
  ...
  Widget _buildOrderCard(GPendingOrder order, int index) {
    return Container(
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: Icon(Icons.assignment, color: warningOrange, size: 24),
                    ),
